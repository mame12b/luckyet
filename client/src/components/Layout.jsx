  ...
  return (
    <div className="min-h-screen flex flex-col bg-bg text-text">
      ...
      <div className="bg-burgundy text-white text-center text-[11px] sm:text-xs py-1.5 px-3">
        <span className="hidden sm:inline">🎰 Launch draw is live — iPhone 15 Pro Max + 100k ETB · </span>
        <span className="sm:hidden">🎰 Launch draw is live · </span>
        <Link to="/draws" className="underline font-semibold">Buy a ticket →</Link>
      </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            {user ? (
              <div className="relative" ref={profileRef}>
                ...
                {profileOpen && (
                  <div className="absolute right-0 mt-2 w-56 bg-white border border-border rounded-lg shadow-card overflow-hidden">
                    ...
                    <div className="py-1">
                      <Link to="/dashboard" onClick={() => setProfileOpen(false)} className="block px-4 py-2 text-sm hover:bg-surface">Dashboard</Link>
                      <Link to="/my-tickets" onClick={() => setProfileOpen(false)} className="block px-4 py-2 text-sm hover:bg-surface">My tickets</Link>
                      <Link to="/my-payments" onClick={() => setProfileOpen(false)} className="block px-4 py-2 text-sm hover:bg-surface">My payments</Link>
                    </div>
              ...
              <>
                <Link to="/login" className="hidden sm:inline-block text-sm font-semibold text-text-muted hover:text-text px-3 py-2">Login</Link>
            ...
            <nav className="flex-1 overflow-y-auto p-2">
              ...
              {user && (
                <>
                  ...
                  <Link to="/dashboard" className="block px-4 py-3 rounded-md text-base font-semibold text-text hover:bg-surface">Dashboard</Link>
                  <Link to="/my-tickets" className="block px-4 py-3 rounded-md text-base font-semibold text-text hover:bg-surface">My tickets</Link>
                  <Link to="/my-payments" className="block px-4 py-3 rounded-md text-base font-semibold text-text hover:bg-surface">My payments</Link>
                </>
              )}
            </nav>
      ...
      <footer className="bg-burgundy text-white pt-10 pb-6 px-4 sm:px-6">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8 mb-6">
            <div className="col-span-2 md:col-span-1">
              ...
              <p className="text-white/70 text-xs leading-relaxed">
                Quantum-verifiable lottery for the Ethiopian & Eritrean diaspora.
              </p>
            </div>
            <div>
              <div className="font-bold mb-2 text-amber-300 text-xs uppercase tracking-wider">Explore</div>
              <ul className="space-y-1.5 text-white/80 text-sm">
                <li><Link to="/draws" className="hover:text-white">Buy</Link></li>
                <li><Link to="/results" className="hover:text-white">Results</Link></li>
                <li><Link to="/how-it-works" className="hover:text-white">How it works</Link></li>
              </ul>
            </div>
            <div>
              <div className="font-bold mb-2 text-amber-300 text-xs uppercase tracking-wider">Help</div>
              <ul className="space-y-1.5 text-white/80 text-sm">
                <li>FAQ</li>
                <li>Support</li>
            ...
            <div>
              <div className="font-bold mb-2 text-amber-300 text-xs uppercase tracking-wider">Connect</div>
              <ul className="space-y-1.5 text-white/80 text-sm">
                <li>TikTok</li>
                <li>Telegram</li>
                <li>Instagram</li>
          ...
          <div className="border-t border-white/10 pt-4 flex flex-wrap items-center justify-between gap-2 text-xs text-white/60">
            <div>© {new Date().getFullYear()} LuckyET</div>
            <div className="flex items-center gap-3">
              <span className="bg-white/10 px-2 py-0.5 rounded font-bold">18+</span>
