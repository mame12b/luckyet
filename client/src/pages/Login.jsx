...
export default function Login() {
  ...
  const submit = async (e) => {
    ...
    if (pinValue.length !== 6) { setError(t("auth.common.enterPassword")); return; }
    ...
    try {
      ...
      // Backend error stays in English per design decision
      setError(resp?.message || t("auth.login.loginFailed"));
    ...
    <div className="bg-surface min-h-[80vh] flex items-start justify-center px-4 py-8 sm:py-12">
      <div className="w-full max-w-md">
        <div className="bg-white border border-border rounded-2xl p-5 sm:p-7 shadow-card">
          ...
          <p className="text-text-muted text-sm mb-5">{t("auth.login.subtitle")}</p>
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-semibold mb-2">{t("auth.common.passwordSixDigits")}</label>
              ...
              <div className="bg-danger-light text-danger text-xs px-3 py-2 rounded-md">
                ...
                {remainingAttempts !== null && remainingAttempts <= 2 && remainingAttempts > 0 && (
                  <div className="mt-1 font-semibold">
                    {t("auth.login.attemptsRemaining", { count: remainingAttempts })}
                  </div>
                )}
              </div>
            )}
